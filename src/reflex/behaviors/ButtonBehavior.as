    ...
    public class ButtonBehavior extends Behavior implements IBehavior
    {
        ...
        override public function set target(value:IEventDispatcher):void
        {
            if(value != null)
            {
                ...
                value.addEventListener(ButtonEvent.STATE_UP, onStateUp);
                value.addEventListener(ButtonEvent.STATE_OVER, onStateOver);
                value.addEventListener(ButtonEvent.STATE_DOWN, onStateDown);
            }
            super.target = value;
        }
